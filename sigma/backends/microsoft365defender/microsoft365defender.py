        ...
        **kwargs,
        ...
        if self.processing_pipeline:
            self.processing_pipeline = (
                microsoft_365_defender_pipeline(transform_parent_image)
                + self.processing_pipeline
            )
        else:
            self.processing_pipeline = microsoft_365_defender_pipeline(
    ...
    def convert_condition_as_in_expression(
        ...
        list_nonwildcard = self.list_separator.join(
            [
                ...
                if (
                    ...
                    and not arg.value.contains_special()
                )
        ...
        if list_nonwildcard:
            as_in_expr = self.field_in_list_expression.format(
                field=field, op=op1, list=list_nonwildcard
            )
        wildcard_exprs_list = []
        if list_wildcards:
            for arg in list_wildcards:
                ...
                if arg[
                    1:-1
                ].contains_special():  # Wildcard in string, not at start or end.
                    ...
                    expr = f"{self.token_separator}{self.and_token}{self.token_separator}".join(
                        [
                            self.contains_expression.format(
                                field=field,
                                value=self.convert_value_str(SigmaString(x), state),
                            )
                            for x in arg.s
        ...
        wildcard_exprs = f"{self.token_separator}{op2}{self.token_separator}".join(
            wildcard_exprs_list
        )
